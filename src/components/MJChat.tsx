...
const MJChat = () => {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: displayContent || "Thank you for staying with us." }]);
    } catch (e: any) {
      setMessages((prev) => [
        ...prev,
        {
            ...
            >
              {messages.map((msg, i) => (
                ...
                >
                  ...
                  >
                    {msg.role === "assistant" ? (
                      <div className="prose prose-sm max-w-none [&_p]:m-0 [&_ul]:my-1 [&_li]:my-0 [&_a]:text-accent [&_a]:underline [&_a]:font-medium">
                        <ReactMarkdown
                          components={{
                            a: ({ node, ...props }) => (
