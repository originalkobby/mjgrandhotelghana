...
async function fetchGuests() {
  // Only show guests that came from the booking engine's guest details form,
  // i.e. guests that have at least one booking linked to them.
  const { data } = await supabase
    .from("guests")
    .select("id, full_name, email, phone, vip, created_at, preferences, bookings!inner(id)")
    .order("created_at", { ascending: false })
    .limit(200);
  // Dedupe & strip the joined bookings field
  const seen = new Set<string>();
  const guests: Guest[] = [];
  for (const row of (data ?? []) as any[]) {
    if (seen.has(row.id)) continue;
    seen.add(row.id);
    const { bookings: _b, ...rest } = row;
    guests.push(rest as Guest);
  }
  return guests;
}
